...
float4 filter(in VSOUT i, in sampler gi, int iteration, bool skip)
{
    ...
    float kernel[4] = {1.5,3.5,7,15};//{SIZE.x, SIZE.y, SIZE.z, SIZE.w}; //1.35 3.75 11.45 18.2
    float sigma_z[4] = {0.7,0.7,0.7,0.7};//{WEIGHTZ.x, WEIGHTZ.y, WEIGHTZ.z, WEIGHTZ.w};// 1.5, 1, 0.6, 0.38
    float sigma_n[4] = {0.75,1.5,1.5,5};//{WEIGHTN.x, WEIGHTN.y, WEIGHTN.z, WEIGHTN.w};// 10,6,6,12
    float sigma_v[4] = {0.035,0.6,1.4,5};//{WEIGHTV.x, WEIGHTV.y, WEIGHTV.z, WEIGHTV.w};// 0.01, 0.29, 5.0 5.0
    ...
    for(float y = -1; y <= 1; y++)
    {        
        //float2 uv = i.uv + float2(x, y) * currkernel * qUINT::PIXEL_SIZE;
